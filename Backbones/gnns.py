...
class GCN(nn.Module):
    def __init__(self,
        ...
        self.gat_layers.append(nn.Linear(dims[-2], dims[-1]))
...
class GCN_SupCon(nn.Module): # for PCL
    def __init__(self,
                 args):
        super(GCN_SupCon, self).__init__()
        dims = [args.d_data] + args.GCN_args['h_dims'] + [args.n_cls]
        self.dropout = args.GCN_args['dropout']
        self.gat_layers = nn.ModuleList()
        for l in range(len(dims)-2):
            self.gat_layers.append(GCNLayer(dims[l], dims[l+1]))
        # self.gat_layers.append(nn.Linear(dims[-2], dims[-2]))
