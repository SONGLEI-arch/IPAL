...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", type=str, default='CoraFull-CL', help='Products-CL, Reddit-CL, Arxiv-CL, CoraFull-CL, CS-CL')
    ...
    parser.add_argument('--method', type=str,
                        choices=["bare", 'lwf', 'gem', 'ewc', 'mas', 'twp', 'jointtrain', 'ergnn', 'joint', 'Joint', 'ncil', 'polo', 'efc', 'yooop', 'fecam'], default="bare",
    ...
    parser.add_argument('--GCN-args', default={'h_dims': [128, 128], 'dropout': 0.0, 'batch_norm': False}) # 0.0
    parser.add_argument('--GIN-args', default={'h_dims': [256], 'dropout': 0.0})
    
    parser.add_argument('--ncil_args', type=str2dict, default={'alpha': 1.0, 'beta': 0.1, 'S': 100, 'K': 10}) 

    parser.add_argument('--fecam_args', type=str2dict, default={'Na': None})
    parser.add_argument('--yooop_args', type=str2dict, default={'feature_per_step': 10})
    parser.add_argument('--polo_args', type=str2dict, default={'budget': [10]})
    parser.add_argument('--efc_args', type=str2dict, default={'budget': [10]})
    parser.add_argument('--ergnn_args', type=str2dict, default={'budget': [10], 'd': [0.05, 0.5, 5.0], 'sampler': ['CM']}, 
                        help='sampler options: CM, CM_plus, MF, MF_plus')
    parser.add_argument('--lwf_args', type=str2dict, default={'lambda_dist': [0.1, 1.0, 10.0], 'T': [0.2, 2.0, 20.0]})
    parser.add_argument('--twp_args', type=str2dict, default={'lambda_l': [100., 10000.], 'lambda_t': [100., 10000.], 'beta': [0.01, 0.1]}) 
    parser.add_argument('--ewc_args', type=str2dict, default={'memory_strength': [1., 100., 10000., 1000000.]})
    parser.add_argument('--mas_args', type=str2dict, default={'memory_strength': [1., 100., 10000., 1000000.]})
    parser.add_argument('--gem_args', type=str2dict, default={'memory_strength': [0.05, 0.5, 5], 'n_memories': [10, 100, 1000]})
    ...
    parser.add_argument('--data_path', type=str, default='./store/processed', help='the path to processed data (splitted into tasks)') # processed_long
    ...
    parser.add_argument('--load_check', type=strtobool, default=True, help='whether to check the existence of processed data by loading')
    ...
    method_args = {'ergnn': args.ergnn_args, 'lwf': args.lwf_args, 'twp': args.twp_args, 'ewc': args.ewc_args,
                   ...
                   'ncil': args.ncil_args, 'polo': args.polo_args, 'efc': args.efc_args, 'yooop': args.yooop_args, 
                   'fecam': args.fecam_args}
    ...
    AP_best, AF_best, name_best = 0, 0, None
    ...
    for hyp_params in hyp_param_list: # 
        ...
        if args.dataset == 'CoraFull-CL':
            args.n_cls_per_task = 10
        elif args.dataset == 'CS-CL':
            args.n_cls_per_task = 2
        elif args.dataset in ['Arxiv-CL', 'Reddit-CL']:
            args.n_cls_per_task = 5
        else:
            args.n_cls_per_task = 3
        ...
        if np.mean(AP_dict[hyp_params_str]) > AP_best:
            ...
            AF_best = np.mean(AF_dict[hyp_params_str])
        ...
        with open(
                f'{args.result_path}/log.txt',
                'a') as f:
            f.write('|val| AP_mean:{},AP_std:{},AF_mean:{},AF_std:{}\n'.format(
                np.mean(AP_dict[hyp_params_str]), 
                np.std(AP_dict[hyp_params_str]),
                np.mean(AF_dict[hyp_params_str]), 
                np.std(AF_dict[hyp_params_str]),
            ))
    ...
    if args.perform_testing:
        ...
        with open(
                ...
                'a') as f:
            f.write(f'{name_best}\n')
            f.write('|test| AP_mean:{},AP_std:{},AF_mean:{},AF_std:{}\n\n'.format(
                np.mean(AP_t), 
                np.std(AP_t),
                np.mean(AF_t), 
                np.std(AF_t),
