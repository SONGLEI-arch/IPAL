...
def assign_hyp_param(args, params):
    ...
    if args.method == 'ncil':
        args.ncil_args = params
    if args.method == 'polo':
        args.polo_args = params
    if args.method == 'efc':
        args.efc_args = params
    if args.method == 'yooop':
        args.yooop_args = params
    if args.method == 'fecam':
        args.fecam_args = params
...
def compose_hyper_params(hyp_params):
    ...
    for hk in hyp_params:
        hyp_param_list_ = []
        hyp_p_current = hyp_params[hk] if isinstance(hyp_params[hk],list) else [hyp_params[hk]] # 变成list
