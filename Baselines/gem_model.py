    ...
    def observe(self, args, g, features, labels, t, train_ids, ids_per_cls, dataset):
        ...
        if len(self.observed_tasks) > 1:
            ...
            dotp = torch.mm(self.grads[:, t].unsqueeze(0),
                            self.grads.index_select(1, indx)) # [1,n] @ [n,T-1] = [1,T-1]
            if (dotp < 0).sum() != 0:
                project2cone2(self.grads[:, t].unsqueeze(1),
                              self.grads.index_select(1, indx), self.margin) # [n,1], [n,T-1], 0.5
                # copy gradients back
    ...
    def observe_class_IL_batch(self, args, g, dataloader, features, labels, t, train_ids, ids_per_cls, dataset):
        ...
        if t != self.current_task:
            ...
            for old_task_i in self.observed_tasks[:-1]:
                ...
                collator = dgl.dataloading.NodeCollator(subgraph, self.memory_data[old_task_i], args.nb_sampler)
                self.old_dataloaders[old_task_i] = torch.utils.data.DataLoader(collator.dataset, 
